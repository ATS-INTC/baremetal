...
for _ in range(10):
    ...
    # print("throughput:" + str(count / TIME_OUT))
    # print("avarage delay:" + str(sum(delays) / count) + "us")
...
if os.path.exists(path + "/" + MODE + ".dat"):
    with open(path + "/" + MODE + ".dat", mode='a', encoding='utf-8') as file:
        content = "{:-5d}\t\t\t{:07.3f}\t\t\t\t{:07.3f}".format(MTU, avarage, std_deviation)
        file.write(content + "\n")
else:
    os.mkdir(path)
    os.chmod(path, stat.S_IRWXO + stat.S_IRWXG + stat.S_IRWXU)
    with open(path + "/" + MODE + ".dat", mode='w', encoding='utf-8') as file:
        file.write("Frame_size(Bytes)\tAvarage(us)\t\t\tStd_deviation\n")
        content = "{:-5d}\t\t\t{:07.3f}\t\t\t\t{:07.3f}".format(MTU, avarage, std_deviation)
        file.write(content + "\n")
